...
author:
  ...
  - name: Bob Rudis
    ...
    email:  author2@work
  - name: Jay Jacobs
    affiliation: Magical Python Land
    address:
    - line 1
    - line 2
    email:  author3@work
abstract: >
  ...
  a series of packages - \CRANpkg{webreadr}, \CRANpkg{urltools}, \CRANpkg{iptools} and \CRANpkg{rgeolocate} - designed to efficiently read in, parse
...
As a statistically-oriented programming language, R is commonly used by these same researchers for data analysis, testing and reporting - but it lacks tools designed for the kinds of data sources and formats that are encountered when dealing with access logs. In this article we review the use cases for particular operations over web data, the limitations in base R when it comes to performing those operations, and a suite of R packages designed to overcome them: \CRANpkg{webreadr} \citep{webreadr}, designed for reading access logs in,
\CRANpkg{urltools} \citep{urltools}, for URL manipulation, \CRANpkg{iptools} \citep{iptools} for handling IP addresses, and \CRANpkg{rgeolocate} \citep{rgeolocate} for direct IP geolocation.
...
To solve this common problem in analysing request logs, the \CRANpkg{urltools} [@urltools] package was created. This contains a function,
<code>url_decode</code>, which decodes URLs - but relies on vectorised, compiled code. Benchmarking the two approaches against each other shows that the \CRANpkg{urltools} implementation is approximately 60-70 times faster over large datasets. Again using **microbenchmark**, if we compare the vectorised decoding of 1,000,000 URLs with \CRANpkg{urltools} against `URLdecode` in a `vapply` loop, we see a 60-70 times speed improvement of urltools over base R:
...
A vector of URLs passed into `url_parse` produces a data.frame, with one column for each of the IETF-supported components, and empty strings representing components that could not be found. Additionally, influenced by the style of the *lubridate* package \citep{lubridate}, `urltools` contains functions to get or *set* individual components:
...
As a result of this functionality, urltools provides a package to make URL manipulation faster, easier and far more accessible, reducing the burden associated with munging access logs or similar datasets and allowing a researcher to get to the statistical analysis faster.

## IP manipulation and geolocation

Access logs also contain IP addresses - unique numeric values that identify a particular computer or network in the context of the internet.

<!-- Bob, I'll rely on you to fill out bits on how iptools is useful for this kind of work; I tend to primarily rely on rgeolocate since my ip-needs are more limited. --> 
...
With these concerns in mind, we wrote the \CRANpkg{rgeolocate} package. Through \pkg{httr} this contains convenient, vectorised bindings
