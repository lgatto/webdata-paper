...
Working with these values allows an analyst to validate their data (by
checking for false or spoof addresses) and is a necessary prerequisite
to the use of some IP geolocation systems (covered later) which require
the numeric forms of IP addresses (\samp{3232235521}) rather than the
standard \dfn{dotted-decimal} form (\samp{192.168.0.1}) found in access
logs.
...
(and more). Designed around combined code, it is extremely fast (for
that code that does not rely on internet connections) and boasts a range
of features.
