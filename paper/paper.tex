...
explain and demonstrate a series of packages - \CRANpkg{webreadr},
\CRANpkg{urltools}, \CRANpkg{iptools} and \CRANpkg{rgeolocate} -
designed to efficiently read in, parse and munge access log data,
allowing researchers to handle it easily.
}
...
\CRANpkg{webreadr} package. This contains user-friendly equivalents to
...
\CRANpkg{urltools} {[}@urltools{]} package was created. This contains a
...
that the \CRANpkg{urltools} implementation is approximately 60-70 times
faster over large datasets. Again using \textbf{microbenchmark}, if we
compare the vectorised decoding of 1,000,000 URLs with
\CRANpkg{urltools} against \texttt{URLdecode} in a \texttt{vapply} loop,
we see a 60-70 times speed improvement of urltools over base R:
...
\CRANpkg{urltools} contains \texttt{url\_parse} - which does the same
